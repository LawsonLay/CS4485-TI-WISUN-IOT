...
function getMACAddress(targetIP) {
  ...
  getRequest.on('response', getResponse => {
    ...
    if (getResponse.payload.length >= 8) {
      ...
      macAddressState = payload.readBigUInt64LE(0);
      const nodes = getTopology().graph.nodes;
      const node = nodes.find(node => node.data.id === targetIP);
      ...
      if (node) {
        if (!node.data.time || date.getTime() > node.data.time) {
          node.data.time = date.getTime();
          node.data.macAddressState = macAddressState.toString();
          console.log(node.data.macAddressState);
        }
      }
    }
  });
 ...
 * @param {canonical ipAddr} targetIP
 */
function getCount(targetIP) {
  const reqOptions = {
    observe: false,
    host: targetIP,
    pathname: 'upd_num_cars',
    method: 'get',
    confirmable: 'true',
    retrySend: 'true',
    options: {},
  ...
  getRequest.on('response', getResponse => {
    ...
    if (getResponse.payload.length > 0) {
      const payload = getResponse.payload;
      // byte read for (uint8_t) with 0 offset
      countState = payload.readUInt8(0);
      const nodes = getTopology().graph.nodes;
      const node = nodes.find(node => node.data.id === targetIP);
      ...
      if (node) {
        if (!node.data.time || date.getTime() > node.data.time) {
          node.data.time = date.getTime();
          node.data.countState = countState;
          console.log(node.data.countState);
        }
      }
    }
  });
...
function postCount(targetIP) {
  ...
  putPayload = [];
  //newValue is within min,max of uint8
...
module.exports = {getLEDStates, postLEDStates, getRSSIValues, getOADFirmwareVersion, startOAD, getCount, postCount,getMACAddress};
