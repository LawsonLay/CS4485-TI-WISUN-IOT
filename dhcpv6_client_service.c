 ...
 */
 ...
 // CC4485 Added Vendor Class Option 16 to Solicit message
...
dhcp_address_get:
    ...
    const char *vendor_class_str = "light switch";
    ...
    // Add option 16 (Vendor Class) to requested options
    uint8_t req_options[] = {DHCPV6_OPTION_VENDOR_CLASS};
    uint8_t req_options_cnt = 1;

    // Calculate payload length with vendor class option
    payload_len = libdhcpv6_solication_message_length(srv_data_ptr->clientDUID.duid_length, add_prefix, req_options_cnt) + option16_total_len;
    ...
    solPacket.requestedOptionCnt = req_options_cnt;
    solPacket.requestedOptionList = req_options;
