...
export default class Topology extends React.Component<TopologyProps, TopologyState> {
    ...
    this.cy.on('mouseover', 'node', e => {
      ...
      this.setState((prevState: TopologyState) => {
        return {
          ...
          countState: node.data('countState'),
          macAddressState: node.data('macAddressState'),
        };
      });
    });
    this.cy.on('mouseout', 'node', e => {
      this.setState((prevState: TopologyState) => {
        return {...prevState, greenLEDState: null, redLEDState: null,countState: null, macAddressState:null};
      });
    });
  ...
  render() {
    ...
    const formattedMACAddress = formatMacAddress(this.state.macAddressState);
    // LED State Info
    const displayLEDStates = this.state.greenLEDState != null && this.state.redLEDState != null;

    const displayCountState = this.state.countState != null;
    
    const displayMACAddressState = this.state.macAddressState != null;
    ...
    return (
      <div style={{width: '100%', position: 'relative'}}>
        {(displayRSSI || displayLEDStates || displayCountState || displayMACAddressState) && (
          <div
            style={{
              position: 'absolute',
              top: 20,
          ...
          >
            ...
            {displayCountState && (
              <InfoMessageTooltipCard
                name={'Car Count'}
                additionalDescriptions={[`Number of Cars: ${this.state.countState}`]}
              />
            )}
            {displayMACAddressState && (
              <InfoMessageTooltipCard
                name={'MAC Address'}
                additionalDescriptions={[`${formattedMACAddress}`]}
              />
            )}
          </div>
        )}
