...
# CoAP ev charger: update number of cars plugged in
COAP_UPDATE_NUM_CARS_URI = "upd_num_cars" 
                ...
                if pkt.upper_layer_protocol.type == ipv6.IPV6_NEXT_HEADER_ICMP:
                    ...
                    if udp_pkt.header.dst_port == COAP_PORT:
                        ...
                        # LED PUT/POST response
                            ...
                            #Handle ev charge CoAP message
                            if h.code == ipv6.COAP_METHOD_CODE_POST:
                                print("Number of cars plugged in: {}".format(list(p.payload)))
                                if h.type == ipv6.COAP_TYPE_CON:
                                    try:
                                        srcIPAddress = pkt.ipv6_header.destination_address
                                        # Send an acknowledgement to the CoAP Node
                                        coap_req = self.ipv6_factory.build_coap_request(format(srcIPAddress), pkt.ipv6_header.source_address, ipv6.COAP_TYPE_ACK, ipv6.COAP_RSP_CODE_CONTENT, None, payload=None, msg_id=(h.msg_id+1), tkl=h.tkl, token=h.token)
